...
const faqs = [
  ...
  },
  {
    ...
    answer: 'ממש לא. השיטה שלי מבוססת על תזונה גמישה ואורח חיים בריא שניתן להתמיד בו לאורך זמן. אני בנה לך תפריט עשיר, מגוון וטעים שכולל מאכלים שאתה אוהב, מותאם לסדר היום ולסגנון החיים שלך, בלי שום הימנעות קשוחה או הרעבה מיותרת.'
  },
  {
    question: 'האם הליווי מתאים גם למתחילים גמורים?',
    answer: 'מתאים במיוחד. אני מפרק איתך את התהליך לצעדים פשוטים וברורים. אני בנה לך תוכנית אימונים הדרגתית, ומלמד אותך בדיוק איך לבצע כל תרגיל בצורה בטוחה ויעילה עם סרטוני הסבר מפורטים וליווי טכני אישי.'
  }
];
  ...
  return (
    <main dir="rtl" className="min-h-screen bg-[#070708] text-white font-sans overflow-x-hidden selection:bg-amber-500/30 selection:text-amber-300">
      ...
      {/* ==========================================
          HERO (מסך ראשי)
         ========================================== */}
      <section className="relative min-h-screen flex flex-col items-center justify-center text-center px-6 pt-40 pb-32 overflow-hidden">
      ...
      {/* ==========================================
          WHY ME (כרטיסיות זכוכית יוקרתיות)
         ========================================== */}
      <section id="why-me" className="relative py-44 px-6 border-t border-neutral-900/50 bg-[#070708]">
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center mb-32">
            <span className="text-xs md:text-sm font-bold tracking-[0.3em] text-amber-500 uppercase mb-4 block">THE ADVANTAGE</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {advantages.map((item, index) => (
              ...
              >
                <div className="text-5xl filter drop-shadow-[0_0_15px_rgba(245,158,11,0.3)]">{item.icon}</div>
                <h3 className="text-2xl font-extrabold text-white tracking-tight">{item.title}</h3>
                <p className="text-neutral-400 text-base md:text-lg leading-relaxed">{item.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ==========================================
          HOW IT WORKS
         ========================================== */}
      <section id="how-it-works" className="relative py-44 px-6 border-t border-neutral-900/50 bg-[#050506]">
        <div className="max-w-6xl mx-auto relative z-10">
          <div className="text-center mb-32">
            <span className="text-xs md:text-sm font-bold tracking-[0.3em] text-amber-500 uppercase mb-4 block">THE PROCESS</span>
            <h2 className="text-4xl md:text-6xl font-black tracking-tight text-white mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            {steps.map((step, index) => (
              <div 
                key={index}
                className="relative flex gap-8 p-8 md:p-10 rounded-3xl border border-white/5 bg-neutral-950/40 hover:border-amber-500/20 transition duration-500 shadow-xl"
              >
                <div className="font-mono text-5xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-b from-amber-500 to-transparent select-none">{step.num}</div>
                <div className="flex flex-col gap-4">
                  <h3 className="text-2xl font-extrabold text-white">{step.title}</h3>
                  <p className="text-neutral-400 text-base md:text-lg leading-relaxed">{step.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ==========================================
          SMART SURVEY (זכוכית קריסטל אמיתית)
         ========================================== */}
      <section id="survey-section" className="relative py-44 px-6 border-t border-neutral-900/50 min-h-[850px] flex items-center justify-center bg-[#070708]">
          ...
          <div className="bg-white/[0.02] backdrop-blur-2xl rounded-3xl p-8 md:p-16 border border-white/10 shadow-[0_0_50px_rgba(245,158,11,0.08)]">
            ...
            {surveyStep === 1 && (
              <div className="flex flex-col gap-8">
                ...
                <input
                  ...
                  className="w-full p-5 rounded-2xl bg-[#0d0d0f] border border-white/10 text-white text-lg focus:outline-none focus:border-amber-500 transition duration-300"
                />
                <button
                  disabled={!name}
                  onClick={() => setSurveyStep(2)}
                  className="py-5 bg-white text-black font-bold text-lg rounded-2xl disabled:opacity-50 transition"
                >
                  המשך
                </button>
              </div>
            ...
            {surveyStep === 2 && (
              <div className="flex flex-col gap-8">
                ...
                <input
                  ...
                  className="w-full p-5 rounded-2xl bg-[#0d0d0f] border border-white/10 text-white text-lg focus:outline-none"
                />
                <div className="flex gap-4">
                  <button onClick={() => setSurveyStep(1)} className="w-1/3 py-5 bg-transparent rounded-2xl text-white border border-white/10">חזור</button>
            ...
            {surveyStep === 3 && (
              <div className="flex flex-col gap-8">
                ...
                <div className="flex gap-4">
                  <button onClick={() => setSurveyStep(2)} className="w-1/3 py-5 bg-transparent rounded-2xl text-white border border-white/10">חזור</button>
                ...
                <div className="flex flex-col gap-4">
                  {['חיטוב וירידה באחוזי שומן', 'עלייה במסת שריר וכוח', 'שיפור סיבולת וכושר כללי'].map((option) => (
                    <button
                      ...
                      className={`w-full p-5 rounded-2xl text-right border transition-all duration-300 ${goal === option ? 'border-amber-500 bg-amber-500/10 text-amber-400 font-bold' : 'border-white/10 bg-[#0d0d0f] hover:bg-white/5'}`}
                    >
                      {option}
                    </button>
                  ))}
                </div>
                <button onClick={() => setSurveyStep(3)} className="py-5 bg-transparent border border-white/10 rounded-2xl text-white">חזור</button>
              </div>
            )}
                ...
                <div className="flex flex-col gap-4">
                  {['חדר כושר', 'בבית עם ציוד בסיסי', 'בחוץ / פארק (משקל גוף)'].map((option) => (
                    <button
                      ...
                      className={`w-full p-5 rounded-2xl text-right border transition-all duration-300 ${location === option ? 'border-amber-500 bg-amber-500/10 text-amber-400 font-bold' : 'border-white/10 bg-[#0d0d0f] hover:bg-white/5'}`}
                    >
                      {option}
                    </button>
                  ))}
                </div>
                <button onClick={() => setSurveyStep(4)} className="py-5 bg-transparent border border-white/10 rounded-2xl text-white">חזור</button>
              </div>
            )}
                ...
                <div className="flex flex-col gap-4">
                  {['1-2 אימונים בשבוע', '3-4 אימונים בשבוע', '5+ אימונים בשבוע'].map((option) => (
                    <button
                      ...
                      className={`w-full p-5 rounded-2xl text-right border transition-all duration-300 ${frequency === option ? 'border-amber-500 bg-amber-500/10 text-amber-400 font-bold' : 'border-white/10 bg-[#0d0d0f] hover:bg-white/5'}`}
                    >
                      {option}
                    </button>
                  ))}
                </div>
                <button onClick={() => setSurveyStep(5)} className="py-5 bg-transparent border border-white/10 rounded-2xl text-white">חזור</button>
              </div>
            )}
            ...
            {surveyStep === 7 && isAnalyzing && (
              <div className="text-center py-16 flex flex-col items-center justify-center gap-8">
                ...
                <h4 className="text-2xl md:text-3xl font-black text-amber-500 animate-pulse">ANALYZING DATA...</h4>
              </div>
            )}
            ...
            {surveyStep === 8 && (
              <div className="text-center flex flex-col items-center gap-8">
                ...
                <p className="text-neutral-300 text-base md:text-lg leading-relaxed">
                  היי <span className="text-amber-500 font-bold">{name}</span>, הכנתי עבורך ניתוח ראשוני. לחץ על הכפתור למטה כדי לשלוח לי את התוצאות לוואטסאפ ונתחיל לעבוד!
                </p>
                <button
                  onClick={handleWhatsAppSend}
                  className="w-full py-5 bg-[#25D366] text-white font-extrabold text-xl rounded-2xl flex items-center justify-center gap-4 hover:shadow-[0_0_30px_rgba(37,211,102,0.4)] transition duration-300"
                >
                  שלח ליצחק בוואטסאפ 🚀
                </button>
              </div>
      ...
      {/* ==========================================
          FAQ (שאלות נפוצות)
         ========================================== */}
      <section id="faq" className="relative py-44 px-6 border-t border-neutral-900/50 bg-[#070708]">
        <div className="max-w-4xl mx-auto relative z-10">
          <div className="text-center mb-32">
            <span className="text-xs md:text-sm font-bold tracking-[0.3em] text-amber-500 uppercase mb-4 block">QUESTIONS</span>
            <h2 className="text-4xl md:text-6xl font-black tracking-tight text-white mb-6">
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <span className={`text-amber-500 transform transition-transform duration-500 ${isOpen ? 'rotate-180' : 'rotate-0'}`}>▼</span>
                  </button>
                  {isOpen && (
      ...
      {/* ==========================================
          FOOTER
         ========================================== */}
      <footer className="relative py-20 px-6 border-t border-neutral-900 text-center bg-[#050506]">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-10">
          <div className="text-right">
            <span className="text-lg font-black tracking-[0.4em] text-amber-500 block mb-2" dir="ltr">YITZHAK AZRAN</span>
